...
public class No_1406_Editor {
    ...
    public static void main(String[] args) throws IOException {
        ...
        for(int c = 0; c < commands; c++) {
            ...
            switch (command) {
                case "L":
                    if(!leftString.isEmpty()) rightString.push(leftString.pop());
                    break;
                case "D":
                    if(!rightString.isEmpty()) leftString.push(rightString.pop());
                    break;
                case "B":
                    if(!leftString.isEmpty()) leftString.pop();
                    break;
                case "P":
                    char letter = st.nextToken().charAt(0);
                    leftString.push(letter);
                    break;
            }
        }

        while(!leftString.isEmpty()) sb.append(leftString.pop());
        sb.reverse();
        while(!rightString.isEmpty()) sb.append(rightString.pop());
