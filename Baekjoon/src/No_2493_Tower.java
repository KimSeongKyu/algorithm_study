...
public class No_2493_Tower {
    ...
    public static void main(String[] args) throws IOException {
        ...
        for(int i = 1; i <= n; i++) {
            int tower = Integer.parseInt(st.nextToken());	// 1.현재 타워의 높이

            while(!towers.isEmpty()) {
                if(towers.peek()[0] > tower) {		// 3.왼쪽에 현재 타워보다 높은 타워가 있는 경우
                    sb.append(towers.peek()[1] + " ");	// 4.수신 타워의 위치 출력
                    break;
                }
                towers.pop();	// 2.왼쪽에 현재 타워보다 높은 타워가 나올 때 까지 반복
            }
            // 5.왼쪽에 현재 타워보다 높은 타워가 없는 경우(수신 타워가 없는 경우) 0 출력
            if(towers.isEmpty()) sb.append(0 + " ");

            towers.push(new int[] {tower, i});	// 6.현재 타워를 다음 타워의 왼쪽 타워로 사용
        }
