...
public class No_2841_AlienGuitarPlay {

    public static void main(String[] args) throws IOException {
        BufferedReader br = new BufferedReader(new InputStreamReader(System.in));
        StringTokenizer st = new StringTokenizer(br.readLine());
        Stack<Integer>[] melodies = new Stack[7];

        for(int i = 0; i < 7; i++) melodies[i] = new Stack<>();
        ...
        for(int i = 0; i < n; i++) {
            ...
            // pop 조건: 현재 누르고 있는 fret보다 입력받은 fret이 작은 경우
            while(!melodies[line].isEmpty() && fret < melodies[line].peek()) {
                melodies[line].pop();
                count++;
            }

            // push 조건: 현재 누르고 있는 fret보다 입력받은 fret이 큰 경우
            // 앞에서 입력받은 fret이 더 작은 경우를 모두 pop했기 때문에 같은지만 비교하면 됨
            if(melodies[line].isEmpty() || fret != melodies[line].peek()) {
