...
트리의 각 정점에는 1번부터 N번까지 번호가 붙어있다. 1번 정점은 '루트 노드' 라고 불리며,
...
public class No_15900_EscapeTree {
    private static int totalEdgeCount = 0;
    private static List<Integer>[] childrenList;
    ...
    public static void main(String[] args) throws IOException {
        ...
        childrenList = new ArrayList[n+1];
        for(int i = 0; i < n+1; i++) childrenList[i] = new ArrayList<>();
        ...
        for(int i = 0; i < n-1; i++) {
            ...
            childrenList[node1].add(node2);
            childrenList[node2].add(node1);
        }

        dfs(1, 0, isVisited);
        if(totalEdgeCount % 2 == 0) System.out.println("No");
        else System.out.println("Yes");
    }

    private static void dfs(int parent, int edgeCount, boolean[] isVisited) {
        isVisited[parent] = true;
        if(childrenList[parent].size() == 1 && parent != 1){
            totalEdgeCount += edgeCount;
            return;
        }

        for(int child : childrenList[parent]) {
            if(!isVisited[child]) dfs(child, edgeCount+1, isVisited);
        }
    }
}